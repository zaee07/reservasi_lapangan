...
class Pembayaran_model extends CI_Model
{
    public function get_pembayaran($cabang_id, $tanggal, $status = null)
    {
        ...
        if ($status) {
            $this->db->where('pembayaran.status_pembayaran', $status);
        }
        return $this->db
    ...
    public function get_riwayat_status($pembayaran_id)
    {
        return $this->db
            ...
            ->join('pembayaran', 'pembayaran.id = riwayat_status_pembayaran.pembayaran_id', 'left')
            ->where('riwayat_status_pembayaran.pembayaran_id', $pembayaran_id)
    ...
    {
        return $this->db->where('booking_id', $booking_id)->update('pembayaran', $data);
    }
